...
stocks = ['SAMSUNG','005930.KS']
stock_factors = ['Volume', 'Adj Close']

df = pd.DataFrame([])
for currency, ticker in currencies.items():
    df[currency] = web.DataReader(ticker, 'yahoo', start, end)['Adj Close']

for factor in stock_factors:
    df[factor] = web.DataReader(stocks[1], 'yahoo', start, end)[factor]

df.dropna()
scaled = pd.DataFrame([])
scaler = MinMaxScaler(feature_range = (0, 1))
for col in df.columns:
    scaled[col] = scaler.fit_transform(df[col].values.reshape(-1, 1))
scaled.dropna()
df.dropna()
scaled_data = scaled.stack()
df = df.stack()
